...
from scorpion.utils import admin_cmd
...
MESAG = (
    ...
    else "`scorpion  security! Please wait for me to approve you. 😊"

)

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "scorpion User"

USER_BOT_WARN_ZERO = "`I had warned you not to spam. Now you have been blocked and reported until further notice.`\n\n**GoodBye!** "

USER_BOT_NO_WARN = (

    "**PM Security ~ Scorpion**\n\nNice to see you here, but  "
...
@scorpion.on(admin_cmd(pattern="a ?(.*)"))

@scorpion.on(admin_cmd(pattern="approve ?(.*)"))
...
@scorpion.on(admin_cmd(pattern="block ?(.*)"))
...
@scorpion.on(admin_cmd(pattern="da ?(.*)"))

@scorpion.on(admin_cmd(pattern="disapprove ?(.*)"))
        ...
        if chat.id == 1654815364:
            ...
            if pmpermit_sql.is_approved(chat.id):
                ...
                await event.edit(
                    ...
                    "[{}](tg://user?id={}) disapproved to PM Don't mess-up again.".format(
...
@scorpion.on(admin_cmd(pattern="listapproved"))
...
async def approve_p_m(event):
    ...
    APPROVED_PMs = "[scorpion] Currently Approved PMs\n"
    ...
    if len(APPROVED_PMs) > 4095:
        ...
        with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
            ...
            await event.client.send_file(
                ...
                caption="[scorpion]Current Approved PMs",
...
async def do_pm_permit_action(chat_id, event):
    ...
    scorpion = await bot.inline_query(mybot, MSG)
...
@scorpion.on(
    ...
    events.NewMessage(
        ...
        incoming=True, from_users=(1654815364, 1096804830, 1686377367)
...
if NEEDIT == "on":
    ...
    @scorpion.on(events.NewMessage(incoming=True))
...
CMD_HELP.update(

    {
        ...
        \nGet help from @scorpion_helpchat."
