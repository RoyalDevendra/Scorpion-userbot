...
async def waifu(animu):
    ...
    try:
        sticcers = await scorpion_bot.inline_query(
            "stickerizerbot",
...
async def hehe(args):
    ...
    user = await scorpion_bot.get_me()
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            photo = io.BytesIO()
            photo = await scorpion_bot.download_media(message.photo, photo)
        elif "image" in message.media.document.mime_type.split("/"):
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            await scorpion_bot.download_file(message.media.document, photo)
            if (
            ...
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            await scorpion_bot.download_file(
    ...
    if photo:
        ...
        pack = 1
        if len(splat) == 3:
            pack = splat[2]  # User sent scorpion_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with scorpion_bot.conversation("Stickers") as conv:
                await conv.send_message("/addsticker")
                await conv.get_response()
                await scorpion_bot.send_read_acknowledge(conv.chat_id)
...
async def ultdestroy(event):
    ...
    if not (
        ...
        return await eor(event, "`Reply to Animated Sticker only`")
    await event.client.download_media(ult, "scorpion.tgs")
    xx = await eor(event, "`Processing...`")
    os.system("lottie_convert.py scorpion.tgs json.json")
    json = open("json.json")
    jsn = json.read()
    json.close()
    jsn = (
    ...
    os.system("lottie_convert.py json.json scorpion.tgs")
    await event.client.send_file(
        event.chat_id,
        file="scorpion.tgs",
        force_document=False,
...
CMD_HELP.update({f"{__name__.split('.')[1]}": f"{__doc__.format(i=HNDLR)}"})
